...
import { useNavigate } from "react-router-dom";
import { getFacultyDetails, updateFacultyProfile } from "../services/api";
import { Plus, Trash2, GraduationCap, Clock, MapPin, Phone, Mail, Building } from 'lucide-react';
import toast from 'react-hot-toast';
...
const EditFacultyProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        toast.success('Profile updated successfully!');
        navigate(userData?.isProctor ? '/proctor-dashboard' : '/professor-dashboard');
      }
    } catch (error) {
      toast.error('Failed to update profile');
    }
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-8 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="text-white">
              ...
              <p className="opacity-80">Joined {new Date(userData.joiningDate).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 p-2 border border-gray-300 rounded-md">
                {editableData.specialization.map((spec, index) => (
                  <span key={index} className="bg-blue-100 px-2 py-1 rounded-md flex items-center">
                    {spec}
                    <button
                      onClick={() => {
                        const newSpec = [...editableData.specialization]
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ',') {
                      ...
                      if (value) {
                        setEditableData({
                          ...editableData,
                          specialization: [...editableData.specialization, value]
                        })
                        e.target.value = ''
                      }
                    }
